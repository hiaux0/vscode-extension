...
export default class AElement extends MozDocElement {
  ...
  constructor() {
      ...
      new BindableAttribute(`Indicates which referrer to send when fetching the URL.`,
        ...
        new Map([
          ...
          ['no-referrer-when-downgrade',
            new Value(`means no Referer: header will be sent when navigating to an origin without HTTPS. This is the default behavior.`)],
          ['origin', new Value(`means the referrer will be the origin of the page, not including information after the domain.`)],
          ['origin-when-cross-origin',
            new Value(`meaning that navigations to other origins will be limited to the scheme,` +
            ` the host and the port, while navigations on the same origin will include the referrer's path.`)],
          ['unsafe-url', new Value(`means the referrer will include the origin and path, but not the fragment, password, or username. ` +
          `This is unsafe because it can leak data from secure URLs to insecure ones.`)],
        ])));
    ...
    this.attributes.set('target',
        new BindableAttribute(`Specifies where to display the linked URL. It is a name of, or keyword for, a browsing context: a tab, ` +
         `window, or <iframe>.`,
          null,
          null,
          null,
          ...
          new Map([
            ...
            ['_blank', new Value(`Load the URL into a new browsing context. This is usually a tab, but users can configure browsers ` +
            `to use new windows instead.`)],
            ['_parent', new Value(` Load the URL into the parent browsing context of the current one. If there is no parent, this ` +
            `behaves the same way as _self.`)],
            ['_top', new Value(`Load the URL into the top-level browsing context (that is, the "highest" browsing context that is an ` +
            `ancestor of the current one, and has no parent). If there is no parent browsing context, this behaves the same way as _self.`)],
          ])));
