...
import { Node, SyntaxKind, SourceFile, createSourceFile, ScriptTarget, ScriptKind, 
  forEachChild, ClassDeclaration, PropertyDeclaration, MethodDeclaration, ParameterDeclaration} from "typescript";
...
export default class ProcessFiles {
  ...
  public async processPath(path: string): Promise<void> {

   // const docParser = new DocumentParser();
    let paths = this.readDirR(Path.join(path, '/src/'));

    for (let path of paths) {
      console.log('processing ' + path );
      let name = Path.basename(path).replace(/\.(ts|js|html)$/, '').split(/(?=[A-Z])/).map(s => s.toLowerCase()).join('-');
      let component = this.findOrCreateWebComponentBy(name);
      ...
      switch(Path.extname(path)) {
        case '.ts':
          console.log('process ts', path);
          var fileContent:string = this.readFileContent(path);
          var result = processSourceFile(path, fileContent, 'typescript');
          ...
          if (result && result.result.length) {
            ...
            for(let classDef of result.result) {
              console.log(classDef.name + classDef.modifiers);
  ...
  private readDirR(dir: string) {
    return FileSystem.statSync(dir).isDirectory()
          ? Array.prototype.concat(...FileSystem.readdirSync(dir).map(f => this.readDirR(Path.join(dir, f))))
          : dir;
  }
