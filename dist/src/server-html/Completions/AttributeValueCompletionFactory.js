...
class AureliaAttributeCompletionFactory {
    static create(elementName, attributeName) {
        switch (elementName) {
            case 'input':
    ...
    static createInputAttributes(attributeName) {
        return this.createInputValueAttributeValues();
    }
    static createInputValueAttributeValues() {
        let result = [];
        ...
        result.push(this.createAttribute(`color`, `A control for specifying a color. A color picker's UI has no` +
            `required features other than accepting simple colors as text.`));
        result.push(this.createAttribute(`date`, `A control for entering a date (year, month, and day, with no time)`));
        result.push(this.createAttribute(`datetime-local`, `A control for entering a date and time, with no time zone.`));
        result.push(this.createAttribute(`email`, `A field for editing an e-mail address. The input value is validated to ` +
        ...
        result.push(this.createAttribute(`month`, `A control for entering a month and year, with no time zone.`));
        result.push(this.createAttribute(`number`, `A control for entering a floating point number.`));
        ...
        result.push(this.createAttribute(`radio`, `A radio button. You must use the value attribute to define the value submitted ` +
            ...
            `at a time.`));
        result.push(this.createAttribute(`range`, `A control for entering a number whose exact value is not important. This type control ` +
      ...
      step: 1`));
        ...
        result.push(this.createAttribute(`search`, `A single-line text field for entering search strings. Line-breaks are automatically ` +
            `removed from the input value.`));
        result.push(this.createAttribute(`submit`, `A button that submits the form.`));
        result.push(this.createAttribute(`tel`, `A control for entering a telephone number. Line-breaks are automatically removed from the ` +
        ...
        result.push(this.createAttribute(`time`, `A control for entering a time value with no time zone.`));
        result.push(this.createAttribute(`url`, `A field for editing a URL. The input value is validated to contain either the empty string or ` +
            ...
            `pseudo-classes are applied as appropriate.`));
        result.push(this.createAttribute(`week`, `A control for entering a date consisting of a week-year number and a week number with no time zone.`));
        return result;
    }
    static createAttribute(name, documentation) {
        return {
            documentation: documentation,
            kind: 12 /* Value */,
            label: name,
            insertText: vscode_languageserver_types_1.SnippetString.create(name)
        };
    }
}
